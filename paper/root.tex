...
by a flexible sensor placed in the air column. The sensor changes its electrical resistance according to the bend caused by the push of the air. Therefore, the flex sensor bend in percentage is assumed as the controlled variable, and the fan speed in percentage is a manipulated variable. The goal was to control the sensor bend to a steady-state value and reject the disturbance.
