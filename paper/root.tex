...
To provide experimental results presenting the proposed tools, the case study was realized on a simple dynamical SISO system called Flexy2 (TODO: Fig). The actuator is a fan that propels an air column in an upward vertical direction. The power of the airflow is measured
by a flexible sensor placed in the air column. The sensor changes its electrical resistance according to the bend caused by the push of the air. Therefore, the flex sensor bend in percentage is assumed as the controlled variable, and the fan speed in percentage is a manipulated variable. The goal was to control the flex sensor bend to a steady-state value and reject the disturbance.
...
As the controlled as well as the manipulated variable were set in percentage, their values were constrained in the MPC optimization problem from $0\%$ to $100\%$. Considering the steady-state values where the model was linearized, i.e., $ u = 40\%$ and $ y = 68\%$, the constraints were set as follows: 
\begin{subequations}
	\label{eq:const_u_y} 
	\begin{eqnarray}
	\label{eq:const_u}
...
\begin{figure}[thpb]
	\centering
	\framebox{\includegraphics[scale=0.5]{images/deltaU_u.eps}}      
