...
To provide experimental results presenting the proposed tools, the case study was realized on a simple dynamical SISO system called Flexy2~\cite{flexy2} (TODO: Fig). The actuator is a fan that propels an air column in an upward vertical direction. The power of the airflow is measured
...
The model of the system was obtained through experimental identification. The matrices of the nominal state-space system, discretized with sampling time $T_\mathrm{s} = 0.01$\,s are:
\begin{subequations}
	\label{eq:model_A_B} 
	\begin{eqnarray}
		A &=& \begin{bmatrix}
...
As the controlled as well as the manipulated variable were set in percentage, their values were constrained in the MPC optimization problem from $0\%$ to $100\%$. Considering the steady-state values where the model was linearized, i.e., $ v^\mathrm{s} = 40\%$ and $ b^\mathrm{s} = 68\%$, the constraints were set as follows: 
\begin{subequations}
	\label{eq:const_u_y} 
	\begin{eqnarray}
	\label{eq:const_u}
...
As the uncertainties in the model were considered, the additive disturbance was constrained as:
\begin{eqnarray}
	\label{eq:const_w}
	-1\% \le w \le 1\%.
\end{eqnarray}
...
Last but not least, the prediction horizon $N$ was set to 30 steps. The tube MPC controller was
constructed in MATLAB 2020b programming environment,
using toolboxes YALMIP R20210331, MPT 3.2.1, MPTplus,
and solver Gurobi 9.1.1. Tube MPC was executed on
CPU AMD Ryzen 7 PRO 4750U, 1.7 GHz with 16 GB RAM. After designing the tube explicit model predictive controller, the disturbance rejection control problem was investigated. The control results can be seen in Fig.~\ref{fig:deltaU_y} for the controlled variable, i.e., the flex sensor bend $b$. In Fig.~\ref{fig:deltaU_u}, the corresponding trajectory of manipulated variable is depicted, i.e., the trajectory of fan speed $v$. The aim was to drive the flex sensor bend to the steady state $ b^\mathrm{s} = 68\%$, while rejecting the effect of the two disturbances occuring at times 10\,s and 20\,s. 


\begin{figure}
	\begin{center}
		\includegraphics[width=0.5\textwidth]{images/deltaU_y.eps}
...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.5\textwidth]{images/deltaU_u.eps}
