...
\begin{subequations}
	...
	\begin{eqnarray}
		...
		\label{eq:tmpc_rpi}
		&\,& \mathrm{s.t.} \quad x(t) - \hat{x}_{0} \in \mathbb{T} , \\
		\label{eq:tmpc_model}
		&\,& \qquad  \hat{x}_{k+1} = A \hat{x}_{k} + B \hat{u}_{k} , \\
		\label{eq:tmpc_constraints_state}
		&\,& \qquad \hat{x}_{k} \in \mathbb{X} \ominus \mathbb{T} , \\
		\label{eq:tmpc_constraints_terminal}
		&\,& \qquad \hat{x}_{N} \in \mathbb{X}_{\mathrm{N}}, \\
		\label{eq:tmpc_constraints_input}
		&\,& \qquad \hat{u}_{k} \in \mathbb{U} \ominus K \, \mathbb{T} , \\
		\label{eq:tmpc_constraints_input_delta_k}
		&\,& \qquad \Delta \hat{u}_{k} \in \mathbb{U}_{\Delta} \ominus K \, \mathbb{T} , \\
		\label{eq:tmpc_constraints_input_delta_0}
		&\,& u(t)- u(t_{-}) + K ( x(t) - \hat{x}_{0} ) \in \mathbb{U} , \quad
	\end{eqnarray}
\end{subequations}
%	where $\hat{x}_{0}$ and $\hat{u}_{k}$ are the predicted states and inputs, respectively, for all  $k = 1, \ldots, N-1$, where $N$ denotes the prediction horizon.
for all steps $\forall k = 1, \dots, N-1$ along the prediction horizon $N$.
